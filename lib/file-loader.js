...
export class FileLoader {
    ...
    static getFiles(directory, ignorePaths = []) {
        ...
        return files.map((filePath) => {
            ...
            return FileLoader.getFileDef(directory, innerFilePath);
        });
    }
    ...
    static getModuleFile(appName, moduleName, fileName) {
        const dir = appName + "/" + moduleName;
        const fullDirectory = "./app/" + dir;
        return FileLoader.getFileDef(fullDirectory, fileName);
    }

    static getFileDef(filePath, requestedFileName) {
    ...
    static async asObject(fd) {
        if (fd && fd.suffix == "js") {
            try {
                ...
                Log.error("If error file missing try NODE_DEBUG=esm");
                Log.error(e);
            }
        }
