...
export class Request {
                ...
                if (header.indexOf("multipart") >= 0) {
                    ...
                    data.forEach((field) => {
                        if (field.type) {
                            ...
                            formData[field.name] = this._normalize(abToString(field.data));
                        }
                    });
                } else {
    ...
    _normalize(value) {
        ...
        if (this._isNumeric(value)) return Number(value);
...
export class Response {
    ...
    static from(x) {
        const c = new Response(x._app, x._res, x._req);
    ...
    status(statusCode) {
        ...
        if (this._headers["Content-Type"]) {
            r = Response.from(this)
        } else {
            r = this.contentType("plain/text");
        }
    ...
    declarative() {
        ...
        var res = new DeclarativeResponse();
        // res.writeStatus("" + (this._statusCode ?? 200));
        ...
        // var length = Buffer.byteLength(this._content);

        // const bytes = this._content;
        // instructions.push(0, length & 0xff, (length >> 8) & 0xff, ...bytes);


        var full = this._req.getUrl();
        // console.log(full);
        this._app.any(full,
            // new Uint8Array(instructions).buffer
