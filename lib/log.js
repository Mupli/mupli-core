...
export class Logger {
    ...
    info(msg, ...data) {
        if (_config["INFO"]) {
            ...
            console.log(date() + " INFO  -- " + this.fileAndLine + " : " + message);
            logError(msg);
        }
    }
    warn(msg, ...data) {
        if (_config["WARN"]) {
            ...
            console.log(
                color,
                date() + " WARN  -- " + this.fileAndLine + " : " + message
            );
            logError(msg, color);
        }
    }
    ...
    error(msg, ...data) {
        if (_config["ERROR"]) {
            ...
            console.log(
                color,
                date() + " ERROR -- " + this.fileAndLine + " : " + message
            );
            logError(msg, color);
        }
    }
...
export class Log {
     ...
     */
    static create() {
        return new Logger(traceCaller(2));
    }
...
function _stringify(el) {
    return JSON.stringify(el, (key, v) => {
        if (typeof v == "string" && v.length > 60) return v.substring(0, 60).trim() + "...";
        return v
    })
}
