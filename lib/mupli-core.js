...
class Mupli {
    ...
    async _buildRoutes(appName) {
        let resultRoutes = this._routes;

        // copy routes
        resultRoutes[appName] = {};

        const all = this._routeBuilders[appName].map((module) => {
            return module.routes(appName, this._servicesCtx[appName]);
        });
    ...
    async _loadModules(appName, rootPath, arch, modulesNames) {
        ...
        await this._onServices(appName, multiModules);
        ...
        await this._buildRoutes(appName);
    }
