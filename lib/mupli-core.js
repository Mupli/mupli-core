...
class Mupli {
    ...
    async _buildRoutes(bootDefaultConfig) {
        ...
        // get routes
        const all = this._routeBuilders[appName].map(
            async (moduleAndConfig) => {
                var { module, config } = moduleAndConfig;
                return await module.routes(config, this._servicesCtx[appName]);
            }
        );
    ...
    async _loadModules(appName, rootPath, arch, modulesNames) {
        ...
        // modules inheritance
        const moduleAndConfigSubModulesList = {};

        this._flattenSubModulesWithmodules(
    ...
    _flattenSubModulesWithmodules(results, moduleList, parentConfig) {
        ...
        const modAndConf = moduleList
            .filter((m) => !results[CoreUtils.getModuleConfigId(m)])
            .flatMap((module) => {
                const paths =
    ...
    _updgradeAction(appName, path) {
        ...
        return async (_res, _req, context) => {
            try {
                ...
                me._handleException(e);
                new Response(_res, _req).status(500).end();
            }
        };
    }
