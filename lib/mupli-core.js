...
class Mupli {
    ...
    async _loadModules(appName, rootPath, arch, modulesNames) {
        ...
        const orginalModules = this._findModules(modulesNames);
        let multiModules = orginalModules.map((m) => ({
            module: m,
            config: config,
        }));

        // modules inheritance
        const parentModules = this._getParentModules(orginalModules, appName);

        multiModules = [...parentModules, ...multiModules];

        //init + execute
        await this._executeInit(multiModules);
        await this._executeCreateServices(multiModules, appName);

        this._registerModuleExtentions(appName, orginalModules);
        this._executeModuleExtentions(appName, orginalModules);

        await this._onServices(appName, orginalModules);

        // PREPARE -----------
        this._prepareContexts(orginalModules, appName);
        this._prepareMiddlewares(multiModules, appName);
        this._prepareWsMiddlewares(multiModules, appName);
        // Routes
    ...
    _getParentModules(multiModules, appName) {
        return multiModules
            .filter((module) => module.modules)
            .flatMap((rootModule) => {
                // await this._loadModules(appName, m.appPath, m.arch, );

                return this._findModules(rootModule.modules).map((m) => ({
                    module: m,
                    config: {
                        appName,
                        appPath: rootModule.appPath,
                        localModules: this._getLocalModules(rootModule.arch, appName),
                    },
                }));
            });
    ...
    async _executeCreateServices(multiModules, appName) {
        ...
        const services = multiModules
            .map((m) => m.module)
            .filter((module) => module.services);
