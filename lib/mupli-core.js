...
import { BootConfig, Ctx, EventService } from "./ctx.js";
...
class Mupli {
     ...
     */
    module(module) {
        if (!module) throw new Error("Module undefined");

        if (typeof module == "function") {
            module = module();
        }
        if (Array.isArray(module)) throw new Error("Array passed - use module(s) method");
    ...
    async _loadModules(appName, rootPath, arch, modulesNames) {
        ...
        const rootModuleList = this._findModules(modulesNames, appName);
    ...
    _flattenSubModulesWithmodules(results, moduleList, parentConfig) {
        ...
        modAndConf
            ...
            .forEach((maAndConfig) => {
                ...
                const moduleModules = this._findModules(module.modules, appName);
    ...
    _findModules(modulesNames, appName) {
        return modulesNames
            ...
            .map((moduleName) => {
                ...
                if (m == undefined)
                    throw new MupliError(
                        "Missing module with name :" + moduleName + " for appName: " + appName
                    );
                return m;
            });
    }
            ...
            if (this.onErrorModules[appName] && e.constructor) {
                ...
                if (errorsHandlers) {
                    ...
                    if (errorsHandlers.length > 0) {
                        ...
                        if (action) {
                            Log.warn("Show 404 for: {}", ctx.req.path())
    ...
    _handler = {
        get(target, propKey, receiver) {
            ...
            throw new MupliError("Service locator cannot find value " + propKey)
        },
    };
