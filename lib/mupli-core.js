...
class Mupli {
    ...
    async listen(port) {
        ...
        for (const appName in this.apps) {
            ...
            let localModules = [];
            if (arch == "modular") {
                localModules = FileLoader.getLocalModuleNames(appName);
            }
            ...
            const initRes = multiModules
                ...
                .map((module) => {
                    ...
                    return module.init(appName, config);
                });
            ...
            this._middlewares[appName] = CoreUtils.createActionFromArray(
                CoreUtils.getFromModules(
                    ...
                    (m) => {
                        ...
                        return m.middlewares(appName, config);
                    }
                )
            );
            ...
            multiModules.forEach((module) => {
                handlerListKeys
                    ...
                    .forEach((methodName) => {
                        ...
                        Objects.resolvePromise(
                            handlerList[methodName](results)
                        );
                    });
            });
            ...
            app.any("/*", async (res, req) => {
                try {
                    ...
                    await me._dispatch(req, res).catch((e) => {
                        Log.error("Core request _dispatch unexpected error " + e);
                        Log.error(e);
                    });
                    // });
                    // me.test(res)
